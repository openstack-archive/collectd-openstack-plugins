...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", privileged: false, inline: <<-SHELL
    ...
    if [ -r "$PROXY_FILE" ]; then
      ...
      export no_proxy=$(ifconfig | grep 'inet addr:' | cut -d':' -f2 | cut -d' ' -f1 | tr '\n' ',')$no_proxy
      python /vagrant/tools/get_proxy_env.py > "$TMP_PROXY_FILE"
    ...
    sudo chown -fR vagrant.vagrant /opt/stack
    sudo ln -sfn /vagrant /opt/stack/collectd-ceilometer-plugin
    ...
    STACKING_FILE="/vagrant/.stacking"
    if [ -f $STACKING_FILE ]; then
      ./unstack.sh && rm -fR $STACKING_FILE || true
    fi
    touch $STACKING_FILE
    ./stack.sh
