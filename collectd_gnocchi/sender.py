...
from collectd_gnocchi.keystone_light import ClientV2 as keystoneClientV2
from collectd_gnocchi.keystone_light import KeystoneException
from collectd_gnocchi.settings import Config
import json
import logging
import requests
...
class Sender(object):
            ...
            try:
                ...
                if self._keystone is None:
                    ...
                    self._keystone = keystoneClientV2(
                        ...
                        username="gnocchi", #cfg.OS_USERNAME,
                        password="password", #cfg.OS_PASSWORD,
                        tenant_name="service", #cfg.OS_TENANT_NAME
                    )
                ...
                # self._url_base = "{}/v1/resource/generic/%s/metric/%s/measures".format(endpoint)
    ...
    def send(self, metername, payload, unit):
        ...
        if result.status_code == HTTP_NOT_FOUND:
            LOGGER.debug("Received 404 error when submitting %s sample, creating a new metric",
                          metername)
    ...
    def _get_metric_id(self, metername, unit):
        ...
        try:
            ...
            endpoint = self._get_endpoint("gnocchi")
            meter_ids[metername] = self._create_metric(metername, endpoint, unit)
    ...
    def _perform_request(cls, url, payload, auth_token):
        ...
        # request headers
        headers = {#'X-User-Id': '79eace1a9c5e416481888a95b9803a09',
                   #'X-Project-Id': '79eace1a9c5e416481888a95b9803a09',
                   'X-Auth-Token': auth_token,
                   'Content-type': 'application/json'}
        # perform request and return its result
        response = None
        try:
            LOGGER.debug("Performing request to: %s with data=%s and headers=%s",
                         url, payload, headers)
