...
from collectd_ceilometer.common.sender import Sender
...
class GnocchiSender(Sender):
    """Sends the JSON serialized data to Gnocchi"""

    def __init__(self):
        """Create the GnocchiSender instance
    ...
    def _handle_http_error(self, exc, metername,
        ...
        if response.status_code == Sender.HTTP_NOT_FOUND:
            ...
            if result.status_code == Sender.HTTP_CREATED:
                LOGGER.debug('Result: %s', Sender.HTTP_CREATED)
            else:
