...
class CollectdLogHandler(logging.Handler):
    ...
    def __init__(self, collectd, level=logging.NOTSET):
        ...
        hooks = sorted(
            six.iteritems(self.collectd_log_hooks),
            key=lambda e: e[1])
        ...
        self._levels = levels = tuple(level for _, level in hooks)
        ...
        self._hooks = tuple(getattr(collectd, name) for name, _ in hooks)
