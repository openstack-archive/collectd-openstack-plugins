...
class TestSampleContainer(unittest.TestCase):
    ...
    def test_sample_container_init(self):
        ...
        self.assertEqual({}, container._data)

    def test_sample_container_add(self):
        """Test adding an element to SampleContainr
        ...
        retval = container.add("key1", ["value1"], 5)
    ...
    def test_sample_container_add_exceeds_limit(self):
        ...
        # TODO(emma-;-foley): What happens when you add more than one item and
        # the new len >>> limit; expect the overflow to be in the sample
        container = SampleContainer()

        self.assertEqual(container._data, defaultdict(list))

        retval = container.add("key1", ["1", "2", "3", ], 2)

        self.assertEqual(retval, ["1", "2", "3", ])
        self.assertEqual([], container._data["key1"])

    def test_sample_container_reset(self):
        """Test resetting the contents of a meter entry in Sample Container
        ...
        container.add("key1", expected["key1"], 42)
        container.add("key2", expected["key2"], 42)

        self.assertEqual(expected, container._data)

        retval = container.reset()
