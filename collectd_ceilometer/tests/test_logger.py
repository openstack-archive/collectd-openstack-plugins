...
class TestCollectdLogHandler(unittest.TestCase):
    ...
    def test_filter_debug(self):
        collectd, record = self._test_filter_when_enabled(logging.DEBUG)
        self.assertIs(collectd.debug, record.hook)

    def test_filter_info(self):
        collectd, record = self._test_filter_when_enabled(logging.INFO)
        self.assertIs(collectd.info, record.hook)

    def test_filter_warning(self):
        collectd, record = self._test_filter_when_enabled(logging.WARNING)
        self.assertIs(collectd.warning, record.hook)

    def test_filter_error(self):
        collectd, record = self._test_filter_when_enabled(logging.ERROR)
        self.assertIs(collectd.error, record.hook)

    def test_filter_fatal(self):
        collectd, record = self._test_filter_when_enabled(logging.FATAL)
        self.assertIs(collectd.error, record.hook)

    def _test_filter_when_enabled(self, level):
        collectd = mock_collectd()
        handler = CollectdLogHandler(collectd=collectd)
        record = mock_record(msg="a_message", level=level)

        result = handler.filter(record)
        self.assertTrue(result)
        return collectd, record
