...
class CollectdService(CollectdServicePeer):
    ...
    def __init__(self, configuration=''):
        ...
        if '{module_path}' in configuration:
    ...
    def has_collectd(cls):
        ...
        if has_collectd is None:
            configuration = '<LoadPlugin "python">\n'\
                            '  Globals true\n'\
                            '</LoadPlugin>\n'

            conf_file, conf_path = tempfile.mkstemp()
            try:
                os.write(conf_file, configuration)
                subprocess.check_output(['collectd', '-C', conf_path, '-t'])
            except Exception:
                has_collectd = False
...
class CollectdServicePlugin(CollectdServicePeer):
    ...
    def post(self, *args, **kwargs):
        self.send_message('post {} {}', args, kwargs)
    ...
    def activate(cls, *args, **kwargs):
        ...
        cfg.VERBOSE = 0
        cfg.BATCH_SIZE = "50"
        cfg.OS_AUTH_URL = "http://192.168.4.3/identity_v2_admin/v2.0"
        cfg.CEILOMETER_URL_TYPE = "internalURL"
        cfg.CEILOMETER_TIMEOUT = "1000"
        cfg.OS_USERNAME = "ceilometer"
        cfg.OS_PASSWORD = "password"
        cfg.OS_TENANT_NAME = "service"

        import mock
        import requests
        service = cls(*args, **kwargs)
        mock.patch.object(
            requests, 'post', side_effect=service.post)
