...
# 
#         self.exceptions = exceptions = queue.Queue()
#         def record_exception(*args, **kwargs):
#             exceptions.put_nowait(sys.exc_info())
# 
#         LOGGER.exception.side_effect = record_exception
# 
#     def tearDown(self):
#         unittest.TestCase.tearDown(self)
#         self.assert_has_no_exceptions()
# 
#     def assert_has_exception(
#             self, expected_type=Exception, expected_value=mock.ANY):
#         _, exception, _ = self.exceptions.get_nowait()
#         self.assertIsInstance(exception, expected_type)
#         self.assertEqual(expected_value, str(exception))

    def assert_has_no_exceptions(self):
        try:
            exc_type, exc_value, exc_tb = self.exceptions.get_nowait()
        except queue.Empty:
            pass
        else:
