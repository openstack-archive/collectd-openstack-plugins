...
from collectd_ceilometer.tests.base import match_string
...
class TestCollectdPython(unittest.TestCase):
    ...
    def test_run_cpu_write(self):
        ...
        self.assertReceiveMessage(
            ...
            "type='cpu',type_instance='user',plugin='cpu',plugin_instance='0',"
            "host='vagrant-ubuntu-trusty-64',time=*,interval=10.0,values=*)")

    def test_collectd_plugin(self):
        coniguration = """
        <Plugin python>
        ...
        LoadPlugin "cpu"
        """
        self.run_collectd(configuration=coniguration)
        self.assertReceiveMessage('init')
        self.assertReceiveMessage('read')
    ...
    def assertReceiveMessage(self, expected, *args, **kwargs):
        ...
        self.assertEqual(match_string(expected), actual)
