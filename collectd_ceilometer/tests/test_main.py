...
class TestMain(unittest.TestCase):
    ...
    @mock.patch.object(main, 'ROOT_LOGGER', autospec=True)
    def test_main(self, ROOT_LOGGER, CollectdLogHandler, Config, Plugin):
        """Verify that the callbacks are registered properly"""

        collectd = mocked.collectd()

        # When main function is called
        main.main(collectd=collectd)
    ...
    @mock.patch.object(main, 'ROOT_LOGGER', new_callable=Logger, name='me')
...
class TestPlugin(unittest.TestCase):
    ...
    @mock.patch('requests.post', spec=callable)
    @mock.patch.object(sender, 'ClientV2', autospec=True)
    @mock.patch.object(main, 'LOGGER', autospec=True)
