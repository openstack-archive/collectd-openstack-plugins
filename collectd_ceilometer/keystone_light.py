...
class ClientV3(object):
    ...
    def refresh(self):
        ...
        try:
            ...
            self._services = resp_data['catalog']
            self._services_by_name = {service['name']: service for service in self._services}
            self._auth_token = resp.headers['X-Subject-Token']
        except (TypeError, KeyError, ValueError) as e:
            LOG.error("Error processing response for capturing ceilometer keystone service")
            ...
            if region:
                ...
                for ep in endpoints:
                    if ep['interface'] in urlkey:
                        return ep["url"].rstrip('/')
            raise ValueError("No valid endpoints found")
        except (KeyError, ValueError) as e:
            LOG.error("Error while processing endpoints")
