...
# License for the specific language gov2/alarmsverning permissions and
...
class Sender(object):
    ...
    def send(self, metername, severity, resource_id):
        ...
        alarm_name = self.get_alarm_name(metername, resource_id)
        state = self.get_alarm_state(severity)
        endpoint = self._get_endpoint("aodh")

        # check for and/or get alarm_id
        update, alarm_id = self._get_alarm_id(alarm_name, severity,
                                              resource_id, metername)
        # create request URL
        ...
        if result.status_code == HTTP_UNAUTHORIZED:
            ...
            if auth_token is not None:
                # and try to repos
                if update is True:
                    result = self._perform_update_request(url,
                                                          auth_token, payload)
                else:
                    result = None
        ...
        if result.status_code == HTTP_NOT_FOUND:
            ...
            new, alarm_id = self._get_alarm_id(alarm_name, severity,
                                               resource_id, metername)
    ...
    def _get_alarm_id(self, alarm_name, severity, resource_id, metername):
        ...
        try:
            ...
            self._alarm_ids[alarm_name] = \
                self._create_alarm(endpoint, severity, metername, resource_id)

        return False, self._alarm_ids[alarm_name]

    def _create_alarm(self, endpoint, severity, metername, resource_id):
        ...
        payload = json.dumps({"state": self.get_alarm_state(severity),
                              "name": alarm_name,
                              "severity": severity,
                              "type": "threshold",
                              "threshold_rule": rule,
    ...
    def get_alarm_name(self, metername, resource_id):
        """Get the alarm name"""
