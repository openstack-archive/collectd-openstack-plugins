...
class Sender(object):
    ...
    def _authenticate(self):
        ...
        with self._auth_lock:
            ...
            try:
                ...
                self._url_base = "{}/v2/alarms/%s".format(endpoint)
    ...
    def send(self, metername, severity, resource_id, message):
        ...
        # Create unique alarm identifiers and get its type
        alarm_name = self._get_alarm_name(metername, resource_id)
        alarm_type = self._get_alarm_type(message)
        threshold = self._get_threshold(message)

        # store the alarm type with name for future reference
        if alarm_name not in self._alarm_names:
            self._alarm_types[alarm_name] = alarm_type
            self._thresholds[alarm_name] = threshold
        # check for and/or get alarm_id
        update_alarm, alarm_id = self._get_alarm_id(alarm_name, severity,
                                                    metername, alarm_type,
                                                    threshold)
        # send the PUT request if alarm update is required
        if update_alarm is True:
            # create request URL
        ...
        if result.status_code == HTTP_NOT_FOUND:
            ...
            # create alarm (endpoint,alarm_name)
            update_alarm, alarm_id = self._get_alarm_id(
                alarm_name, severity, metername, alarm_type)
    ...
    def _get_alarm_id(self, alarm_name, severity, metername,
        ...
        try:
            return True, self._alarm_ids[alarm_name]
            ...
            if alarm_name not in self._alarm_names:
                ...
                self._alarm_ids[alarm_name] = \
                    self._create_alarm(endpoint, severity, threshold,
                                       metername, alarm_name, alarm_type)
        return False, None

    def _create_alarm(self, endpoint, severity, threshold, metername,
                      alarm_name, alarm_type):
        """Create a new alarm with a new alarm_id."""
        url = "{}/v2/alarms/".format(endpoint)

        # Check which type of alarm to create and create payload
    ...
    def _get_alarm_type(self, message):
        """Get the type of alarm."""
        message = message.split()
        # anything other than a threshold notification is an event alarm
    ...
    def _get_threshold(self, message):
        """Get the threshold of Notification for alarm."""
        message = message.split()
        threshold = message[-1]
