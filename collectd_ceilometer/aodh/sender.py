...
class Sender(common_sender.Sender):
    ...
    def _create_request_url(self, metername, **kwargs):
        ...
        request_type, alarm_id = self._new_or_existing_id(alarm_name,
                                                          severity, metername)
        url = self._url_base % (alarm_id)

        return request_type, url
    ...
    def _handle_http_error(self, exc, metername,
        ...
        if response.status_code == common_sender.Sender.HTTP_NOT_FOUND:
            ...
            # create/update alarm
            request_type, alarm_id = \
                self._new_or_existing_id(alarm_name, severity, metername)
    ...
    def _new_or_existing_id(self, alarm_name, severity, metername):
        # check for an alarm and update
        try:
            return "update", self._get_alarm_id(alarm_name)
        ...
        return "post", self._alarm_ids[alarm_name]
    ...
    def _get_payload(self, request_type, metername, **kwargs):
