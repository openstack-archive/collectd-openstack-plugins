...
class Sender(object):
    ...
    def send(self, metername, severity, resource_id):
        ...
        # Create unique alarm identifiers and get its state
        alarm_name = self._get_alarm_name(metername, resource_id)

        # check for and/or get alarm_id
        update_alarm, alarm_id = self._get_alarm_id(alarm_name,
                                                    severity, metername)

        # create the payload for the update requests
        # TODO(helena-mcgough): make the type of alarms and its
        # rules adjustable
