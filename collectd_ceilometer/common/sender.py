...
class Sender(object):
    ...
    def send(self, metername, payload, **kwargs):
        ...
        try:
            ...
            return self._perform_request(payload, url, auth_token, **kwargs)
            ...
            if response.status_code == Sender.HTTP_UNAUTHORIZED:
                ...
                if auth_token is not None:
                    ...
                    return self._perform_request(payload, url,
                                                 auth_token, **kwargs)
            else:
                ...
                if alarm_payload is None:
                    ...
                    self._handle_http_error(exc, metername, alarm_payload,
                                            auth_token, **kwargs)

    def _perform_request(self, payload, url, auth_token, **kwargs):
        """Perform the appropriate update/post request"""
        if payload is not None:
            return self._perform_post_request(url, payload, auth_token)
        else:
            alarm_payload = self._get_alarm_payload(**kwargs)
            return self._perform_update_request(url, auth_token, alarm_payload)
